...
class Solution3 {
    fun mergeTwoLists(l1: Main1.ListNode?, l2: Main1.ListNode?): Main1.ListNode? {
    ...
    fun sortList(head: Main1.ListNode?): Main1.ListNode? {
        // 널인 노드까지 분할되면 아무런 처리 없이 리턴
        if (head?.next == null) return head

        // 러너 기법 활용, 빠른 노드가 끝까지 갈 때 느린(slow) 노드는 중간까지 이동한다.
        var half: Main1.ListNode? = null
        var slow: Main1.ListNode = head
        // 끝까지 가는 빠른 러너는 널이 될 수 있다.
        var fast: Main1.ListNode? = head
